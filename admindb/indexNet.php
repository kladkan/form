...
if (isset($_POST['table_name'])) {
    ...
    $stmt = $pdo->prepare("CREATE TABLE `{$_POST['table_name']}` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(400) NOT NULL,
            `author` char NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8");
    $stmt->execute();
}
...
if (isset($_GET['del_field'])) {
    ...
    $sql->execute();
    header('Location: ./indexNet.php?describe_table='.$_GET['table_name']);
}
...
<body>
    <p><a href="indexNet.php?create_table=create_table">Создать таблицу</a> | <a href="indexNet.php?show_tables=show_tables">Показать все таблицы в базе данных</a></p>

    <?php if (isset($_GET['create_table'])) : ?>
        <form action="indexNet.php" method="POST">
            <fieldset>
            ...
            <tr>
                <?php foreach ($row as $col_name => $value) : ?>
                    ...
                    <?php if ($col_name == 'Type') : ?>
                        <form action="indexNet.php?field_name=<?= $row['Field'] ?>&table_name=<?= $_GET['describe_table'] ?>" method="POST">
                ...
                <?php endforeach; ?>
                <td><a href="indexNet.php?del_field=<?= $row['Field'] ?>&table_name=<?= $_GET['describe_table'] ?>">Удалить</a></td>
            </tr>
            <?php endforeach; ?>
        </table>
    <?php endif ?>
