...
if (isset($_GET['del_field'])) {
    ...
    $sql->execute();
    header('Location: ./index.php?describe_table='.$_GET['table_name']);
}
...
<body>
    <p><a href="index.php?create_table=create_table">Создать таблицу</a> | <a href="index.php?show_tables=show_tables">Показать все таблицы в базе данных</a></p>
        ...
        <table width="" border="1" cellpadding="4" cellspacing="0">
            ...
            <tr>
                <?php foreach ($table as $key => $value) : ?>
                    <td>
                        <a href="index.php?describe_table=<?= $value ?>"><?= $value ?></a>
                    </td>
                <?php endforeach; ?>
            </tr>
            <?php endforeach; ?>
            ...
            <tr>
                <?php foreach ($row as $col_name => $value) : ?>
                    ...
                    <?php if ($col_name == 'Type') : ?>
                        <form action="index.php?field_name=<?= $row['Field'] ?>&table_name=<?= $_GET['describe_table'] ?>" method="POST">
                ...
                <?php endforeach; ?>
                <td><a href="index.php?del_field=<?= $row['Field'] ?>&table_name=<?= $_GET['describe_table'] ?>">Удалить</a></td>
            </tr>
            <?php endforeach; ?>
        </table>
    <?php endif ?>
