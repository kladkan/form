...
if (isset($_GET['exit'])) {
    session_destroy();
    header('Location: ./indexNetology.php');
}
echo '<a href="indexNetology.php?exit=exit">Выход</a><br>';
...
if (!isset($_SESSION['user_id'])) {
    if (!empty($_POST['regname']) && !empty($_POST['regpass'])) {
        ...
        if (!empty($row['id'])) { //если существует то
            //echo $row['id'];
            echo 'Логин: '.$_POST['regname'].' - занят! Зарегистрируйтесь под другим именем.<br><a href="indexNetology.php?reg=reg">Зарегистрироваться</a>';
            //unset($_POST);
        } else {
            ...
            print_r($_POST);
            echo '</pre>';*/
            echo '<br><a href="indexNetology.php">Войти</a>';
        }
    }
    if (empty($_POST['authname']) && empty($_POST['authpass'])) {
        if (empty($_POST['regname']) or empty($_POST['regpass'])) {
            echo '<br>Оба поля обязательны для заполнения!<br><a href="indexNetology.php?reg=reg">Зарегистрироваться</a><br><br>';
    ...
    if (!empty($_POST['authname']) && !empty($_POST['authpass'])) {
        ...
        if (!empty($row['id'])) { //если существует то
            ...
            <br><a href="indexNetology.php">Войти заново </a> или
            <br><a href="indexNetology.php?reg=reg">Зарегистрироваться</a>';
            exit;
...
if (isset($_SESSION['user_id']) && isset($_GET['echo_your_tasklist'])) {
    ...
    echo '</pre>';*/
    if (empty($all)) {
        header('Location: ./indexNetology.php?count_task=count_task');
        exit;
    }
...
if (isset($_SESSION['user_id']) && key($_GET) == 'del_task_namder_id') {
    ...
    $stmt->execute();
    header('Location: ./indexNetology.php?echo_your_tasklist=echo_your_tasklist');
}
...
<?php if (isset($_GET['reg'])) : ?>
    <?php if (empty($_POST['regname']) && empty($_POST['regpass'])) : ?>
        <form action="indexNetology.php" method="POST">
        <fieldset>
        <legend>РЕГИСТРАЦИЯ</legend>
            <p>Для регистрации, введите логин и пароль. Если вы зарегистрированы, выполните <a href="indexNetology.php">вход.</a></p>
...
<?php if (!isset($_GET['reg']) && !isset($_POST['regname']) && !isset($_POST['regpass']))  : ?>
    <?php if (!isset($_SESSION['user_id'])) : ?>
        ...
        <legend>ФАРМА ДЛЯ ВХОДА</legend>
            <p>Чтобы войти, введите логин и пароль или пройдите<a href="indexNetology.php?reg=reg"> Регистрацию</a></p>
...
<?php if (isset($_SESSION['user_id'])) : ?>
    <p><a href="indexNetology.php?add_your_task=add_your_task">Добавить дело</a> || <a href="indexNetology.php?echo_your_tasklist=echo_your_tasklist">Вывод списка ваших дел(отсортированных по дате)</a> || <a href="indexNetology.php?echo_assigned_list=echo_assigned_list">Показать делегированные дела</a> || <a href="indexNetology.php?count_task=count_task">Вывести количество дел</a></p>

    <?php if (isset($_GET['add_your_task'])) : ?>
    <form action="indexNetology.php?add_your_task=add_your_task" method="POST">
        <fieldset>
                ...
                <?php foreach ($row as $key => $value) : ?>
                    <td><?php
                        if ($key == 'Выполнено/Невыполнено') {
                            if ($value == 0) {
                                echo '<a href="indexNetology.php?done_on_off=1&task_id='.$row['Удаление дела'].'">Невыполнено</a>';
                            } else {
                                echo '<a href="indexNetology.php?done_on_off=0&task_id='.$row['Удаление дела'].'">Выполнено</a>';
                            }
                        }                        
                        ...
                        ?>
                        <?php if ($key == 'Автор') : ?>
                            <form action="indexNetology.php?echo_your_tasklist=echo_your_tasklist" method="POST">
